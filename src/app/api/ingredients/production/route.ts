      ...
      for (const recipe of recipes) {
        ...
        if (currentStock < requiredQty) {
          const shortage = requiredQty - currentStock;
           ...
           stockErrors.push(
             `Stok ${recipe.childIngredient.name} kurang: butuh ${fmt(requiredQty)} ${recipe.childIngredient.unit}, tersedia ${fmt(currentStock)} ${recipe.childIngredient.unit} (kurang ${fmt(requiredQty - currentStock)}).`
           );
        } else {
